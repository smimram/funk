...
{
  /* returning id + 145 */
  __asm__ __volatile__("addq $145, %rax; iret");
}
...
idt_contents[IDT_SIZE],
  ...
  gdt_contents[] =
{
  [KERNEL_CS/sizeof(*gdt_contents)] = { 0x0000FFFF, 0x00AF9A00 },
...
system_table =
{
  ...
  .base_addr = (unsigned long) gdt_contents,
},
  idt_table =
{
  .limit     = sizeof(idt_contents)-1,
  .base_addr = (unsigned long) idt_contents,
};
